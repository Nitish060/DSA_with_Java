/* Size of largest BST in a Binary Tree */

public class BST10 {
    static class Node
    {
        int data;
        Node left;
        Node right;

        public Node(int data)
        {
            this.data = data;
            this.left = this.right = null;
        }
    }

    static class Info //Info class for storing information of node
    {
        boolean isBST;
        int size;
        int min;
        int max;

        public Info(boolean isBST, int size, int min, int max)
        {
            this.isBST = isBST;
            this.size = size;
            this.min = min;
            this.max = max;
        }
    }

    public static int maxBST = 0; //variable for storing size of largest bst

    public static Info largestBST(Node root) //function for calculating the largest BST
    {
        if(root==null)
        {
            return new Info(true, 0, Integer.MAX_VALUE, Integer.MIN_VALUE);
        }

        Info leftInfo = largestBST(root.left);
        Info rightInfo = largestBST(root.right);
        int size = leftInfo.size + rightInfo.size + 1;

        int min = Math.min(root.data, Math.min(leftInfo.min, rightInfo.min));
        int max = Math.max(root.data, Math.max(leftInfo.max, rightInfo.max));

        if(root.data<=leftInfo.max || root.data>=rightInfo.min) //it checks if node is following the valid BST
        {
            return new Info(false, size, min, max);
        }

        if(leftInfo.isBST && rightInfo.isBST) //it checks if left subtree and right subtree is following the the valid BST
        {
            maxBST = Math.max(maxBST,size);
            return new Info(true, size, min, max);
        }

        return new Info(false, size, min, max);
    }

    public static void main(String[] args) {
        Node root = new Node(50);
        root.left = new Node(30);
        root.left.left = new Node(5);
        root.left.right = new Node(20);

        root.right = new Node(60);
        root.right.right = new Node(70);
        root.right.right.right = new Node(80);
        root.right.right.left = new Node(65);
        root.right.left = new Node(45);
        
        Info info = largestBST(root);
        System.out.println("Largest BST is of size = "+maxBST);
    }
}
